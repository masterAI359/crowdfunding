...
const FAQSection = () => {
  return (
    <section className="bg-[#FF0066] py-12 px-4 md:py-16 min-h-screen">
        ...
        <Accordion type="multiple" className="space-y-3 md:space-y-4">
          {faqs.map((faq, index) => (
            <AccordionItem
              key={index}
              value={`item-${index}`}
              className="bg-white rounded-md px-5 md:px-6 border-0 text-black flex flex-col gap-1 py-4"
            >
              <AccordionTrigger className="text-left font-medium hover:no-underline text-sm md:text-base">
                {faq.question}
              </AccordionTrigger>
              <AccordionContent className="text-muted-foreground text-sm md:text-base px-1">
