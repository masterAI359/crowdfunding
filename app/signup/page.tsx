...
const SignupPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsLoading(false)
      setError(
        err.response?.data?.message || 'サインアップに失敗しました。もう一度お試しください。'
      )
    }
  }
    ...
    <div className="min-h-screen bg-white flex items-center justify-center py-12 px-6 lg:px-8 pt-28">
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="text-center">
          <h2 className="text-3xl font-bold text-gray-900 mb-2">メールアドレスでサインアップ</h2>
        </div>
        ...
        <div className="space-y-6">
          {/* Section Title */}
          <div className="text-center">
            <h3 className="text-sm font-semibold text-gray-700">その他の登録オプション</h3>
          </div>
          ...
          <div className="flex justify-center gap-4">
            ...
            <button
              type="button"
              onClick={() =>
                (window.location.href = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080/api'}/auth/apple`)
              }
              className="w-14 h-14 flex items-center justify-center border border-gray-300 rounded-lg bg-white hover:bg-gray-50 transition-colors shadow-sm"
              aria-label="Appleで登録"
            >
            ...
            >
              ...
              >
                ...
                <path
                  d="M13.5 12h2.5l-1-4h-2.5V7.5c0-.8.2-1.3 1.3-1.3H16V2.1c-.6-.1-1.3-.1-2-.1-2 0-3.4 1.2-3.4 3.4V8H7v4h2.6v8h3.4v-8h2.5z"
                  fill="white"
                />
              </svg>
            </button>
