...
const SellerDashboardPage = () => {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      const [videosData, stats] = await Promise.all([getVideosByOwner(1, 100), getDashboardStats()])
  ...
  const menuStructure = [
      ...
      children: [
        {
          id: 'crowdfunding',
          label: 'クラウドファンディング',
          href: '/user-settings/seller?tab=crowdfunding',
        },
    ...
    },
    {
      ...
      type: 'category' as const,
      children: [{ id: 'payment', label: '支払い', href: '/user-settings/seller?tab=payment' }],
    },
    {
      id: 'contacts',
      label: '連絡先',
      ...
      children: [
        {
          id: 'notifications',
          label: '全ての連絡先',
          href: '/user-settings/seller?tab=notifications',
        },
      ],
    },
    {
          ...
          <div className="flex-1 w-auto">
            {isLoadingData ? (
              ...
              <>
                ...
                {activeTab === 'videos' && (
                  <VideosTab videos={videos} onRefresh={loadDashboardData} isAdmin={isAdmin} />
                )}
                {activeTab === 'products' && <ProductsTab />}
                {activeTab === 'crowdfunding' && <CrowdfundingTab />}
  ...
  useEffect(() => {
    ...
    const generateRevenueData = (salesDataToUse: any[]) => {
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        const dateLabel = currentDate.toLocaleDateString('ja-JP', {
          month: 'numeric',
          day: 'numeric',
        })
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-white border border-gray-200 rounded-lg p-4 relative">
            ...
            {revenueData &&
              ...
              revenueData.current &&
              (() => {
                const previousTotal = revenueData.previous.reduce(
                  (sum: number, d: any) => sum + d.y,
                  0
                )
                const currentTotal = revenueData.current.reduce(
                  (sum: number, d: any) => sum + d.y,
                  0
                )
                if (previousTotal > 0) {
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          {isMounted && (
            <Chart options={mainChartOptions} series={mainChartSeries} type="area" height={350} />
          )}
        </div>
      </div>
            ...
            {videoStats && (
              <div className="grid grid-cols-4 gap-4 mb-6 p-4 bg-gray-50 rounded-lg">
                ...
                <div>
                  <div className="text-sm text-gray-500">コメント数</div>
                  <div className="text-lg font-semibold">
                    {videoStats.commentCount?.toLocaleString() || 0}
                  </div>
                </div>
              </div>
            )}
      ...
      if (uploadedFiles.length > 0) {
        ...
        if (formType === 'edit') {
          ...
          setCreateForm({
            ...
            medias: [
              ...createForm.medias,
              { url: mediaUrl, type: mediaType, order: createForm.medias.length },
            ],
          })
        }
      } else {
...
const NotificationsTab = () => (
  <div className="p-8">
    <h1 className="text-2xl font-bold">全ての通知</h1>
  </div>
)
const CustomerTab = () => (
  <div className="p-8">
    <h1 className="text-2xl font-bold">カスタマー</h1>
  </div>
)
