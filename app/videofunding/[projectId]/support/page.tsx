...
const SupportPage = ({ params: paramsPromise }: { params: Promise<{ projectId: string }> }) => {
  ...
  const handlePurchase = () => {
    ...
    router.push(
      `/videofunding/checkout?projectId=${params.projectId}&rewardIds=${rewardIds}&quantities=${quantitiesStr}`
    )
  }
  ...
  return (
    <div className="min-h-screen bg-white text-black">
      <main className="mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 pb-0">
        ...
        {/* Project Info */}
        <div className="flex flex-col gap-0 max-w-5xl mx-auto mb-6 sm:mb-8 rounded-lg">
          <div className="w-full px-3 py-4 font-bold flex gap-1 items-baseline border-t-2 border-x-2 border-[#ECEBD9] rounded-t-lg">
            <div className="flex items-center gap-3">
              <input
                type="checkbox"
                id={`video-price`}
                className="h-5 w-5 sm:h-6 sm:w-6 text-red-600 rounded focus:ring-red-500 cursor-pointer opacity-80 ml-4"
                checked={isVideoPurchase}
                onChange={() => setIsVideoPurchase(!isVideoPurchase)}
              />
              <label
                htmlFor={`video-price`}
                className="text-[20px]font-bold flex items-center gap-1"
              >
                <span className="text-3xl">45,000</span>
          ...
          <div className="flex flex-col md:flex-row overflow-hidden bg-[#ECEBD9] ">
            ...
            <div className="w-full md:w-2/3 flex flex-col md:flex-row gap-0 p-2">
              ...
              <div className="md:w-2/3 flex flex-col justify-between p-3 sm:p-4">
                <h2 className="text-lg sm:text-xl font-bold tracking-tight mb-2">{video.title}</h2>
                ...
                <div className="flex items-center justify-between gap-2 md:space-x-12">
                  ...
                  <div>
                    <p className="text-xs sm:text-sm text-black text-center">作成者</p>
                    <p className="text-lg sm:text-xl font-bold">
        ...
        <div className="max-w-5xl mx-auto space-y-6 mb-6 sm:mb-8">
          {rewards.map((reward) => (
            <div key={reward.id} className="bg-white border border-[#E9E9E9] rounded-lg">
              <div className="bg-[#ECEBD9] px-10 py-2 rounded-t-lg border-b border-gray-300">
                <h3 className="font-bold text-lg sm:text-xl text-black">{reward.title}</h3>
              </div>
                ...
                <div className="flex flex-col space-y-3 sm:space-y-4 md:col-span-2">
                  <div className="flex items-center gap-3 sm:gap-4">
                    ...
                    <label
                      ...
                      className="text-[20px] text-black/50 font-bold flex items-center gap-1"
                    >
                      シリーズ購入
                    </label>
                  </div>
                  ...
                  <div className="leading-relaxed">
                    {reward.description.map((description, index) => (
                      <p key={index} className="text-sm sm:text-lg text-black text-left md:mx-4">
                        {description}
                      </p>
                    ))}
                  </div>
